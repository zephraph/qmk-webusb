...
//   aw// const { connectToKeyboard } = require('./dist');
...
// const config = HID.devices().filter(({ productId }) => productId === 0x1307)[0];
// (async () => {
// 	try {
// 		const device = new HID.HID(config.path)
// 		console.log('This device works!', JSON.stringify(device))
// 		device.on('data', data => {
// 			console.log(data)
// 			console.log('bytes written', device.write(new Uint8Array([0, 2])))
// 		})
// 	} catch (error) {
// 		console.log(error)
// 	}
...
// var hidstream = new Hidstream({ vendorId: 0xfeed, productId: 0x1307 })
...
const handleIncomingPackets = async () => {
	...
	do {
		try {
			...
			console.log({ cmds })
			cmds.forEach((cmd) => { //// when do we get 2 or more cmds?
				const status = cmd[0]
				let col
				let row
				if (status === 0x00) {
